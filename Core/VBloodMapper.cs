    ...
    public static class VBloodMapper
{
    ...
    public static bool SpawnVBloodBoss(int guidHash, float3 position)
    {
        ...
        try
        {
            ...
            // Note: Skip adding specific VBloodUnit component here to avoid compatibility issues
            VRCore.EM.SetComponentData(entity, new Translation { Value = position });
            VRCore.EM.SetComponentData(entity, new Rotation { Value = quaternion.identity });
    ...
    public static class VBloodUnlockSystem
    {
        ...
        {
            return _vBloodUnlockEnabled.TryGetValue(charEntity, out var enabled) && enabled;
        }
        ...
        public static bool EnableVBloodUnlockMode(Entity character)
        {
            try
            {
                ...
                // Mark as enabled in a simple compatibility-safe way
        ...
        {
            // Compatibility-safe no-op: log action instead of performing engine-specific modifications
            Plugin.Logger?.LogDebug($"[VBloodUnlockSystem] (shim) AddSpellUnlockAbility char={charEntity.Index} slot={slot} prefab={abilityPrefab.GuidHash}");
        }
        ...
        {
            // Compatibility shim - no-op to avoid dependency on game-specific buff APIs
            Plugin.Logger?.LogDebug($"[VBloodUnlockSystem] (shim) UpdateVBloodUnlockBuff buffEntity={buffEntity.Index}");
        }
        ...
        public static bool IsVBloodProgressionUnlocked(Entity character, int guidHash)
        {
            try
            {
                // In arena context, always return true to allow access to all VBlood content
                return true;
            }
            catch (Exception ex)
            {
