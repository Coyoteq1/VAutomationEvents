...
namespace VAuto.Commands
{
    ...
    public static class AutomationCommands
    {
        ...
        public static void SmartCommand(ChatCommandContext ctx, string action, string target = "", string options = "")
        {
            ...
            {
                Plugin.Log?.LogError($"Error in smart command: {ex.Message}");
        ...
        public static void BatchCommand(ChatCommandContext ctx, string action, string targets = "", string options = "")
        {
            ...
            {
                Plugin.Log?.LogError($"Error in batch command: {ex.Message}");
        ...
        {
            var entities = ParseTargets(targets);
            var cleanupType = ParseOptions(options, "type", "memory");

            ctx_reply($"Batch cleaning up {cleanupType} for {entities.Count} targets...");

            int cleaned = 0;
            foreach (var entity in entities)
            {
                if (ApplyCleanup(entity, cleanupType))
                {
                    cleaned++;
                }
            }

            ctx_reply($"Batch cleanup completed: {cleaned}/{entities.Count} entities cleaned");
        }
        ...
        private static void ctx_reply(string message) => Plugin.Log?.LogInfo($"[AutomationCommands] {message}");
        ...
        private static void RegisterEventTrigger(ChatCommandContext ctx, string trigger, string action, int cooldown)
        {
            ...
            Plugin.Instance.Log?.LogInfo($"Registered trigger '{trigger}' for '{action}' with {cooldown}s cooldown");
        }
        #endregion
