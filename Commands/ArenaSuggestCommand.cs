        ...
        public static void SuggestCommand(ChatCommandContext ctx, string category = null)
        {
            try
            {
                if (string.IsNullOrEmpty(category))
                {
                    ...
                    {
                        ctx.Reply($"{index}) {cat.Value.Icon} {cat.Key} - {cat.Value.Suggestions.Count} suggestions");
                        index++;
                    }
                ...
                switch (category)
                {
                    case "spawn":
                        if (action == "glow")
                        {
                            ...
                            ctx.Reply("<color=green>Applied arena glow effect</color>");
                        }
                        break;
                    ...
                    case "arena":
                        if (action == "border")
                        {
                            ...
                            var characterEntity = ctx.Event.SenderCharacterEntity;
                            ArenaGlowService.ApplyArenaBorderGlow(userEntity, characterEntity);
                            ctx.Reply("<color=green>Applied arena border glow</color>");
                        }
                        break;
