...
namespace VAuto.Commands.Player
{
    ...
    public static class CharacterCommands
    {
        ...
        [Command("chcharacter", adminOnly: true, usage: "chcharacter <action> [args]")]
        ...
        private static void CharacterInfo(ChatCommandContext ctx, string target)
        {
            try
            {
                ...
                if (VAuto.Services.Systems.GameSystems.IsPlayerInArena(user.PlatformId))
                {
        ...
        public static void CharacterSwapCommand(ChatCommandContext ctx)
        {
            try
            {
                ...
                // TODO: Implement CharacterSwapService or remove these commands
                ctx.Reply("❌ Character swap service not implemented.");
                ctx.Reply("This feature requires CharacterSwapService to be implemented.");
                return;
                ...
                ctx.Reply("❌ Character swap not implemented.");
                return;
                ...
                // Original code (commented out - unreachable due to return above):
                // if (swapSuccess)
                // {
        ...
        public static void CreateArenaCharacterCommand(ChatCommandContext ctx)
        {
            try
            {
                ...
                if (VAuto.Core.DualCharacterManager.HasDualState(platformId))
                {
                    var state = VAuto.Core.DualCharacterManager.GetState(platformId);
                ...
                // Create dual state
                var dualState = VAuto.Core.DualCharacterManager.GetOrCreateState(platformId, normalCharacter);
        ...
        public static void CharacterStatusCommand(ChatCommandContext ctx)
        {
            try
            {
                ...
                // Arena status
                var inArena = VAuto.Services.Systems.GameSystems.IsPlayerInArena(platformId);
        ...
        public static void CharacterResetCommand(ChatCommandContext ctx, string target = "")
        {
            try
            {
                ...
                VAuto.Core.DualCharacterManager.ResetDualState(targetPlatformId);
        ...
        private static void CharacterOnline(ChatCommandContext ctx, string target)
        {
            try
            {
                ...
                ctx.Reply($"In Arena: {(VAuto.Services.Systems.GameSystems.IsPlayerInArena(user.PlatformId) ? "Yes" : "No")}");
            }
            catch (Exception ex)
            {
            ...
            try
            {
                ...
                switch (action)
                {
                    ...
                    case "exit":
                        ...
                        break;
                    case "status":
                        var inArena = VAuto.Services.Systems.GameSystems.IsPlayerInArena(user.PlatformId);
