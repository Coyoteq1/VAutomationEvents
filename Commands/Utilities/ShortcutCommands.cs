...
using VAuto.Commands.Character;
...
namespace VAuto.Commands.Utilities
{
    ...
    public static class ShortcutCommands
    {
        ...
        public static void HelpCommand(ChatCommandContext ctx, string topic = "")
        {
            ...
            {
                Plugin.Logger.LogError($"Error in help command: {ex.Message}");
                ctx.Reply($"Error: {ex.Message}");
            }
        }

        [Command("sc", "sc <action> [topic]", "Shortcut management helper", adminOnly: false)]
        public static void ShortcutController(ChatCommandContext ctx, string action = "list", string topic = "")
        {
            var key = string.IsNullOrWhiteSpace(topic) ? action : topic;
