...
namespace VAuto.Services
{
    ...
    public class ConveyorService : IService
    {
        ...
        public static void EnableForTerritory(Entity territoryOwner)
        {
            ...
            Instance._log.LogInfo($"[ConveyorService] Enabled conveyor system for territory owner {territoryOwner}");
        }
        ...
        public static void DisableForTerritory(Entity territoryOwner)
        {
            ...
            Instance._log.LogInfo($"[ConveyorService] Disabled conveyor system for territory owner {territoryOwner}");
        }
        ...
        private async Task UpdateLoop()
        {
            while (_isInitialized)
            {
                ...
                {
                    _log.LogError($"[ConveyorService] Error in update loop: {ex.Message}");
                }
        ...
        private async Task UpdateTerritory(ConveyorTerritoryConfig config)
        {
            ...
            {
                _log.LogError($"[ConveyorService] Error updating territory {config.TerritoryOwner}: {ex.Message}");
            }
        }
        ...
        private void RefreshInventories(ConveyorTerritoryConfig config)
        {
            try
            {
                ...
                {
                    if (!em.HasComponent<Name>(entity)) continue;

                    var nameComponent = em.GetComponentData<Name>(entity);
                    var name = nameComponent.Value.ToString();
            ...
            {
                _log.LogError($"[ConveyorService] Error refreshing inventories: {ex.Message}");
            }
        }
                    ...
                    foreach (var item in overflowInventory)
                    {
                        if (receiver.Items.ContainsKey(item.Key))
                        {
                            ...
                            if (needed > 0)
                            {
                                ...
                                if (result.Success)
                                {
                                    _log.LogInfo($"[ConveyorService] Overflow transfer: {result.ItemsTransferred}x {result.ItemName} to {receiver.Name}");
                                }
                            }
                        }
                    }
                ...
                foreach (var buffer in link.Buffers.Where(b => b.NeededAmount > 0))
                {
                    if (link.Sender.Items.ContainsKey(buffer.ItemGuid))
                    {
                        ...
                        if (result.Success)
                        {
                            ...
                            _log.LogInfo($"[ConveyorService] Station transfer: {result.ItemsTransferred}x {result.ItemName} from {link.SenderName} to {link.ReceiverName}");
                        }
                    }
                }
            }
            ...
            foreach (var link in chestLinks)
            {
                foreach (var item in link.Sender.Items)
                {
                    ...
                    if (available > 0)
                    {
                        ...
                        {
                            link.LastTransfer = DateTime.UtcNow;
                            _log.LogInfo($"[ConveyorService] Chest transfer: {result.ItemsTransferred}x {result.ItemName} from {link.SenderName} to {link.ReceiverName}");
                        }
                    }
                }
            }
        ...
        private InventoryType DetermineInventoryType(Entity entity)
        {
            ...
            // Check for station components (simplified)
            if (em.HasComponent<ProcessingStation>(entity))
        ...
        private Dictionary<PrefabGUID, int> GetInventoryItems(Entity entity)
        {
            ...
            try
            {
                ...
                if (!em.HasComponent<Inventory>(entity)) return items;

                var inventory = em.GetComponentData<Inventory>(entity);
            ...
            {
                _log.LogError($"[ConveyorService] Error getting inventory items: {ex.Message}");
                return items;
            }
        }
        ...
        private async Task<TransferResult> TransferItem(Entity fromEntity, Entity toEntity, PrefabGUID itemGuid, int amount)
        {
            ...
            {
                _log.LogError($"[ConveyorService] Error transferring item: {ex.Message}");
