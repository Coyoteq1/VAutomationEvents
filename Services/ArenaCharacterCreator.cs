...
namespace VAuto.Services
{
    ...
    public static class ArenaCharacterCreator
    {
        ...
        private static void MoveToHiddenLocation(Entity arenaCharacter)
        {
            try
            {
                // Default hidden arena location
                var hiddenPos = new float3(-1000, 5, -500);

                // Teleport to hidden location
                MissingServices.TeleportFix.FastTeleport(arenaCharacter, hiddenPos);

                // Freeze the character initially
                DualCharacterManager.FreezeCharacter(arenaCharacter);
        ...
        public static Entity RecreateArenaCharacter(ulong platformId, Entity normalCharacter, Entity userEntity)
        {
            try
            {
                ...
                {
                    // Update dual state
                    DualCharacterManager.InitializeArenaState(platformId, normalCharacter, newArenaCharacter);
                }
