...
namespace VAuto.Services.Systems
{
    ...
    public static class CastleObjectIntegrationService
    {
        ...
        public static bool UpdateCastleObjectPosition(Entity entity, float3 newPosition)
        {
            lock (_lock)
            {
                try
                {
                    ...
                    if (VRCore.EM.HasComponent(entity, new ComponentType(Il2CppType.Of<Translation>())))
                    {
                        VRCore.EM.SetComponentData(entity, new Translation { Value = newPosition });
                    }
                    else
                    {
                        VRCore.EM.AddComponent(entity, new ComponentType(Il2CppType.Of<Translation>()));
                        VRCore.EM.SetComponentData(entity, new Translation { Value = newPosition });
                    }
        ...
        private static float3 GetEntityPosition(Entity entity)
        {
            try
            {
                var translation = new Translation { Value = float3.zero }; // TODO: Implement proper entity translation extraction
        ...
        private static quaternion GetEntityRotation(Entity entity)
        {
            try
            {
                var rotation = new Rotation { Value = quaternion.identity }; // TODO: Implement proper entity rotation extraction
