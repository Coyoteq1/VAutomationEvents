...
namespace VAuto.Services.Systems
{
    ...
    public static class AutoEnterService
    {
        ...
        public static bool TryAutoEnter(Entity user, Entity character, float3 position)
        {
            try
            {
                ...
                return LifecycleService.EnterArena(user, character);
            }
            catch (Exception ex)
            {
        ...
        private static TimeSpan GetCooldownRemaining(ulong platformId)
        {
            lock (_lock)
            {
                if (!_autoEnterPlayers.TryGetValue(platformId, out var data))
