...
namespace VAuto.Services.ECS
{
    ...
    public partial struct AutoEnterSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            try
            {
                var autoEnterService = AutoEnterService.Instance;
                ...
                foreach (var (user, character, transform, entity) in SystemAPI.Query<RefRO<User>, RefRO<PlayerCharacter>, RefRO<LocalTransform>>().WithEntityAccess())
                {
                    var userEntity = entity;
                    var characterEntity = character.ValueRO.LocalCharacter._Entity;
        ...
        private static bool ShouldAutoEnter(ulong platformId, float3 position)
        {
            try
            {
                ...
                if (!AutoEnterService.Instance.IsAutoEnterEnabled(platformId))
                    return false;

                // Check if player is not already in an arena
                if (ArenaZoneService.Instance.IsInArena(position))
                    return false;
    ...
    public partial struct PlayerPositionMonitorSystem : ISystem
    {
        ...
        public void OnUpdate(ref SystemState state)
        {
            try
            {
                if (!AutoEnterService.Instance.IsInitialized)
                    return;
                ...
                foreach (var (user, transform) in SystemAPI.Query<RefRO<User>, RefRO<LocalTransform>>())
                {
                    ...
                    if (ArenaZoneService.Instance.IsInTransitionZone(position))
                    {
        ...
        private static void HandleZoneTransition(ulong platformId, float3 position)
        {
            try
            {
                if (ArenaZoneService.Instance.IsInArena(position))
                {
                    ...
                    EventService.Instance.Trigger(new PlayerArenaEnterEvent { PlatformId = platformId });
                }
                else
                {
                    // Player exited arena
                    ...
                    EventService.Instance.Trigger(new PlayerArenaExitEvent { PlatformId = platformId });
                }
            }
            catch (System.Exception ex)
            {
