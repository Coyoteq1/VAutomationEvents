    ...
    public class RespawnPreventionConfigService
    {
        ...
        public bool IsEnabled
        {
            ...
            set
            {
                ...
                SaveConfig();
                Plugin.Logger?.LogInfo($"[RespawnPreventionConfigService] Respawn prevention {(value ? "enabled" : "disabled")} via configuration");
            }
        }
        ...
        public int DefaultCooldownSeconds
        {
            ...
            set
            {
                ...
                SaveConfig();
                Plugin.Logger?.LogInfo($"[RespawnPreventionConfigService] Respawn prevention default cooldown set to {value} seconds");
            }
        }
        ...
        private void LoadConfig()
        {
            try
            {
                if (!File.Exists(SETTINGS_PATH))
                {
                    ...
                    SaveConfig();
                    Plugin.Logger?.LogInfo("[RespawnPreventionConfigService] Created default respawn prevention configuration");
                    return;
                }
                ...
                Plugin.Logger?.LogInfo($"[RespawnPreventionConfigService] Loaded respawn prevention configuration: Enabled={IsEnabled}, DefaultCooldown={DefaultCooldownSeconds}s");
            }
            catch (System.Exception ex)
            {
                Plugin.Logger?.LogWarning($"[RespawnPreventionConfigService] Failed to load respawn prevention config, using defaults: {ex.Message}");
                _config = GetDefaultConfig();
                SaveConfig();
            }
        }
        ...
        private void SaveConfig()
        {
            ...
            {
                Plugin.Logger?.LogError($"[RespawnPreventionConfigService] Failed to save respawn prevention config: {ex.Message}");
            }
        }
        ...
        public void CleanupExpiredPlayerCooldowns(System.TimeSpan maxAge)
        {
            lock (_lock)
            {
                ...
                {
                    SaveConfig();
                    Plugin.Logger?.LogInfo($"[RespawnPreventionConfigService] Cleaned up {expiredKeys.Count} expired player cooldowns");
                }
            }
        }
    }
