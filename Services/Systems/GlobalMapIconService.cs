...
namespace VAuto.Services.Systems
{
    ...
    public static class GlobalMapIconService
    {
        ...
        // Unique GUID for map icon system
        private static readonly System.Guid MapIconSystemGuid = new System.Guid("A7B3C9D2-E4F1-4A8B-9C5D-6F8E7A1B2C3D");
        ...
        public static void Initialize()
        {
            lock (_lock)
            {
                ...
                try
                {
                    ...
                    Log?.LogInfo($"[GlobalMapIconService] Global map icon service initialized with {MapIconPrefabName} prefab");
                }
                catch (Exception ex)
                {
        ...
        public static void Update(float deltaTime)
        {
            ...
            {
                Log?.LogError($"[GlobalMapIconService] Error in update: {ex.Message}");
            }
        }
        #endregion
        ...
        private static List<UserData> GetAllOnlinePlayers()
        {
            ...
            try
            {
                // Use PlayerService which has better thread safety and error handling
        ...
        private static float3 GetEntityPosition(Entity entity)
        {
            ...
            {
                Log?.LogWarning($"[GlobalMapIconService] Error getting entity position: {ex.Message}");
                return float3.zero;
            }
        }
        ...
        private static quaternion GetEntityRotation(Entity entity)
        {
            ...
            {
                var em = VAuto.Core.Core.EntityManager;
                if (em == null || !em.HasComponent<Rotation>(entity))
                {
            ...
            {
                Log?.LogWarning($"[GlobalMapIconService] Error getting entity rotation: {ex.Message}");
                return quaternion.identity;
            }
        }
        #endregion
        ...
        public static void SetUpdateInterval(float seconds)
        {
            lock (_lock)
            {
                ...
                _timeSinceLastUpdate = 0.0f; // Reset timer
